...
/**
 * Class responsible for generating and storing the randomised spell names and descriptions for each world,
 * which are displayed as glyphs using the SGA font renderer.
 *
 * @since Wizardry 1.1
 */
...
public class SpellGlyphData extends WorldSavedData {
    ...
    /**
     * Returns the spell glyph data for this world, or creates a new instance if it doesn't exist yet.
     * Also checks for any spells that are missing glyph data and adds it accordingly.
     */
    ...
    public void sync(EntityPlayerMP player){
        ...
        List<PacketGlyphData.GlyphEntry> glyphs = new ArrayList<>();

        int id = 0;
        while(id < Spell.getTotalSpellCount()){
            Spell spell = Spell.byNetworkID(id + 1); // +1 because the None spell is not included
            String name = this.randomNames.get(spell);
            String desc = this.randomDescriptions.get(spell);
            if(spell != null){
                glyphs.add(new PacketGlyphData.GlyphEntry(spell.getRegistryName().toString(), name, desc));
            }
            id++;
        }

        PacketGlyphData.Message msg = new PacketGlyphData.Message(glyphs);
